%% Runs parameterization for one or several plsDS 

file_names = ["plsDS"]; % names of mat-files containing plsDS
relax_tail = 600; % relaxation period to fit
...
for fn = 1:numel(file_names)
    ...
    %% ТОЛЬКО ДЛЯ ТЕСТА - обрабатываются только первые два импульса, чтобы скрипт быстро отработал
    % убрать (закомментировать) для обработки всех импульсов для всех soc
    plsDS = plsDS(1:2,:);
    %%
